...
std::string device_info() {
    ...
    for (size_t i = 0; i < info.size(); i++) {
        ret += std::format(
            ...
            wstring_to_string(info[i].description), info[i].vram_size / 1048576,
            info[i].vendor_id);
    }
    return ret;
}
    ...
    for (size_t device_idx = 0; device_idx < info.size(); device_idx++) {
        for (size_t output_idx = 0; output_idx < info[device_idx].size();
            ...
            ret += std::format(
                ...
                output.rotation_angle, output.is_primary ? "True" : "False");
        }
    }
    return ret;
}

std::shared_ptr<DXCamera> create(const int device_idx,
    ...
    const int output_idx_ = output_idx.value_or(-1);
